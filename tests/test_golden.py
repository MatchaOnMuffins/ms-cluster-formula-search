...
class TestGoldenOutputs:
    ...
    def test_peak_1519_all_levels(self, expected_data, level):
        ...
        peak_data = next(
            p for p in expected_data["peaks"] 
            if abs(p["mz"] - 1519.154) < 0.001
        )
...
class TestGoldenFormulas:
    ...
    def test_y2mn4_radical_anion(self, expected_data):
        ...
        peak = next(p for p in expected_data["peaks"] if abs(p["mz"] - 1519.154) < 0.001)
        ...
        match = next(
            (r for r in results 
             if r["formula"] == "Y2Mn4(tBuCOO)10O6H3C1" 
             and r["adduct"] == "[M]−•"),
            None
        )
    ...
    def test_y2mn4_deprotonated(self, expected_data):
        ...
        match = next(
            (r for r in results 
             if r["formula"] == "Y2Mn4(tBuCOO)10O6H4C1" 
             and r["adduct"] == "[M−H]−"),
            None
        )
    ...
    def test_y2mn5_chloride_adduct(self, expected_data):
        ...
        match = next(
            (r for r in results 
             if r["formula"] == "Y2Mn5(tBuCOO)10O4H3C2" 
             and r["adduct"] == "[M+Cl]−"),
            None
        )
    ...
    def test_level3_unique_formulas(self, expected_data):
        ...
        level3_unique = {"Y2Mn5(tBuCOO)10O3H9C0", "Y2Mn5(tBuCOO)10O3H8C0", 
                        "Y1Mn5(tBuCOO)10O9H1C0", "Y1Mn5(tBuCOO)10O9H2C0",
                        "Y2Mn5(tBuCOO)10O0H8C4", "Y2Mn5(tBuCOO)10O0H9C4"}
...
def generate_golden_file(output_path: Path, peaks: list[dict]) -> None:
    ...
    for peak_config in peaks:
        ...
        for level in [1, 2, 3]:
            ...
            peak_data["levels"][str(level)] = [
                normalize_result(r) for r in results
            ]
